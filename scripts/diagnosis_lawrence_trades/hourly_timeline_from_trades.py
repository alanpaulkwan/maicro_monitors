...
def main():
    ...
    # PnL vs theoretical PnL (hourly / daily)
    print("\nPnL vs theoretical PnL (hourly / daily):")

    df_pnl = df_te.copy()
    if not df_pnl.empty:
        df_pnl = df_pnl.sort_values(["sym", "ts_hour"])

        df_pnl["bn_px_next"] = df_pnl.groupby("sym")["bn_px"].shift(-1)
        df_pnl["ret_fwd"] = df_pnl["bn_px_next"] / df_pnl["bn_px"] - 1.0

        mask_ret = (
            df_pnl["equity_usd"].notnull()
            & (df_pnl["equity_usd"] != 0)
            & df_pnl["bn_px"].notnull()
            & df_pnl["bn_px_next"].notnull()
            & df_pnl["actual_weight"].notnull()
        )
        df_pnl = df_pnl[mask_ret].copy()

    if df_pnl.empty:
        print("Not enough data to compute PnL series.")
    else:
        for col in ["weight_t2", "weight_t1"]:
            if col in df_pnl.columns:
                df_pnl[col] = df_pnl[col].fillna(0.0)

        df_pnl["pnl_actual"] = (
            df_pnl["equity_usd"] * df_pnl["actual_weight"] * df_pnl["ret_fwd"]
        )
        df_pnl["pnl_theo_t2"] = (
            df_pnl["equity_usd"] * df_pnl["weight_t2"] * df_pnl["ret_fwd"]
        )
        df_pnl["pnl_theo_t1"] = (
            df_pnl["equity_usd"] * df_pnl["weight_t1"] * df_pnl["ret_fwd"]
        )

        hourly_pnl = (
            df_pnl.groupby("ts_hour")[["pnl_actual", "pnl_theo_t2", "pnl_theo_t1"]]
            .sum()
            .sort_index()
        )

        hourly_pnl["cum_actual"] = hourly_pnl["pnl_actual"].cumsum()
        hourly_pnl["cum_theo_t2"] = hourly_pnl["pnl_theo_t2"].cumsum()
        hourly_pnl["cum_theo_t1"] = hourly_pnl["pnl_theo_t1"].cumsum()
        hourly_pnl["date"] = hourly_pnl.index.date

        daily_pnl = hourly_pnl.groupby("date")[["pnl_actual", "pnl_theo_t2", "pnl_theo_t1"]].sum()

        daily_pnl["cum_actual"] = daily_pnl["pnl_actual"].cumsum()
        daily_pnl["cum_theo_t2"] = daily_pnl["pnl_theo_t2"].cumsum()
        daily_pnl["cum_theo_t1"] = daily_pnl["pnl_theo_t1"].cumsum()

        if len(daily_pnl) >= 2:
            # Full-sample daily correlation
            corr_t2 = daily_pnl["pnl_actual"].corr(daily_pnl["pnl_theo_t2"])
            corr_t1 = daily_pnl["pnl_actual"].corr(daily_pnl["pnl_theo_t1"])

            # Rolling 5-day correlation (using min_periods=3 to see it sooner)
            daily_pnl["roll5_corr_t2"] = (
                daily_pnl["pnl_actual"]
                .rolling(window=5, min_periods=3)
                .corr(daily_pnl["pnl_theo_t2"])
            )
            daily_pnl["roll5_corr_t1"] = (
                daily_pnl["pnl_actual"]
                .rolling(window=5, min_periods=3)
                .corr(daily_pnl["pnl_theo_t1"])
            )

            print("\n▼ Daily PnL correlation (actual vs theoretical):")
            print(f"- T+2 weights (full sample): {corr_t2:.3f}")
            print(f"- T+1 weights (full sample): {corr_t1:.3f}")

            print("\n▼ Rolling 5-day PnL correlation (actual vs theoretical):")
            print(
                daily_pnl["roll5_corr_t2"].tail(10).to_frame("roll5_corr_t2").join(
                    daily_pnl["roll5_corr_t1"].tail(10).to_frame("roll5_corr_t1")
                ).to_string()
            )
        else:
            print("Not enough daily points to compute PnL correlation.")

        print("\nSample of daily PnL and cumulative series:")
